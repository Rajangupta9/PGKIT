...
// Returns ErrNoRows if no row is found. Never mutates the supplied builder.
func QueryOneInto[T any](ctx context.Context, c *Client, b *qb.Builder) (*T, error) {
	b2 := b.Clone()
	if b2 == nil {
		return nil, fmt.Errorf("db/scan: nil builder")
	}
	b2.Limit(1)
...
// TxQueryOneInto executes b.BuildSelect() LIMIT 1 inside a raw pgx.Tx and scans the first row into T.
// Returns ErrNoRows if no row is found. Never mutates the supplied builder.
func TxQueryOneInto[T any](ctx context.Context, tx pgx.Tx, b *qb.Builder) (*T, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("db/scan: tx collect row: %w", err)
	}
	return &result, nil
}
...
func TxInsertInto[T any](ctx context.Context, tx pgx.Tx, b *qb.Builder, data map[string]any) (*T, error) {
	...
	rows, err := tx.Query(ctx, sql, args...)
	if err != nil {
		return nil, fmt.Errorf("db/scan: tx insert query: %w", err)
	}
	defer rows.Close()
	...
	if err != nil {
		...
		return nil, fmt.Errorf("db/scan: tx insert collect: %w", err)
	}
	return &result, nil
}
...
func TxUpdateInto[T any](ctx context.Context, tx pgx.Tx, b *qb.Builder, data map[string]any) (*T, error) {
	...
	rows, err := tx.Query(ctx, sql, args...)
	if err != nil {
		return nil, fmt.Errorf("db/scan: tx update query: %w", err)
	}
	defer rows.Close()
	...
	if err != nil {
		...
		return nil, fmt.Errorf("db/scan: tx update collect: %w", err)
	}
	return &result, nil
}
